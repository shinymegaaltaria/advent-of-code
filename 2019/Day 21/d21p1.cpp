...
#include <map>
using namespace std;
ifstream codein ("day21_in.txt");
ifstream commandin ("d21p1_command.txt");
ofstream debug ("d21p1_debug.txt");
struct intcode{
...
int main(){
    ...
    for(long long int i = 0 ; i < s ; i++){
        if(0 <= pgm.output[i] && pgm.output[i] <= 255) cout << char(pgm.output[i]);
        else cout << pgm.output[i] << endl;
    }
}