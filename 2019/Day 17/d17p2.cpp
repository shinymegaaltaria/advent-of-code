...
#include <queue>
using namespace std;
ifstream codein ("day17_in.txt");
ofstream debug ("d17p2_debug.txt");
ifstream commandin ("commands.txt");
ofstream video ("video.txt");
struct intcode{
...
int main(){
    ...
    while(cpu.halt == false){
        ...
        for(long long int i = 0 ; i < s ; i++) video << char(cpu.output[i]);
        cpu.output.erase(cpu.output.begin(),cpu.output.end());
    }
    long long int s = cpu.output.size();
    for(long long int i = 0 ; i < s - 1 ; i++) video << char(cpu.output[i]);
    cout << cpu.output[s - 1] << endl;
}