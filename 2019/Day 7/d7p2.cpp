...
int main(){
    ...
    do{
        ...
        intcode thrusters[5];
        for(int i = 0 ; i < 5; i++){
            thrusters[i].program = code;
            thrusters[i].pointer = 0;
            thrusters[i].input.push_back(phase[i]);
            thrusters[i].halt = false;
        }
        for(int i = 0 ;thrusters[i%5].halt == false; i++){
            thrusters[i%5].input.push_back(last_output);
            thrusters[i%5] = run(thrusters[i%5]);
            last_output = thrusters[i%5].output[0];
            thrusters[i%5].output.erase(thrusters[i%5].output.begin());
        }
